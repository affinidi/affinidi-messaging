...
pub(crate) async fn messages_received(
    ...
    async move {
        ...
        let to = if let Some(to) = &msg.to {
            if let Some(first) = to.first() {
                ...
                return Err(MediatorError::RequestDataError(
                    session.session_id.clone(),
                    "Message missing valid 'to' field, expect at least one address in array."
                        .into(),
                ));
            }
        } else {
    ...
    // Pull recipient_did and limit from message body
    let message_id_list: Vec<String> = match
        serde_json::from_value::<MessagePickupMessagesReceived>(msg.body.to_owned())
    {
        Ok(body) =>
        body.message_id_list,
        Err(e) =>
        return Err(MediatorError::RequestDataError(
            session.session_id.clone(),
