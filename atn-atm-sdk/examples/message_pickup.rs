...
async fn main() -> Result<(), ATMError> {
    ...
    let mut protocols = Protocols::new(&mut atm);
    ...
    */
    ...
    protocols.message_pickup.toggle_live_delivery(true).await?;
    ...
    let response = protocols
        .message_pickup
        .send_delivery_request(None, None, None, None)
        .await?;
    ...
    let response = protocols
        .message_pickup
        .send_messages_received(None, None, &delete_ids, None)
        .await?;
    ...
    if let SendMessageResponse::WebSocket(response) = response {
        debug!("Trying to get msg-id ({})", response.message_id);
        if let Some((message, _)) = protocols
            .message_pickup
            .live_stream_get(&mut atm, &response.message_id, Duration::from_secs(2))
            .await?
        {
            info!("Message: {:?}", message);
        }
