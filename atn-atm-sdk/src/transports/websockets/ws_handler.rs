...
use atn_atm_didcomm::{Message, UnpackMetadata};
use futures_util::sink::SinkExt;
use http::header::AUTHORIZATION;
use std::{collections::{HashMap, HashSet}, mem::size_of_val};
use tokio::{
    net::TcpStream,
    select,
...
impl MessageCache {
    fn insert(&mut self, message: Message, meta: UnpackMetadata) {
        self.messages.insert(message.id.clone(), (message.clone(), meta));
        self.ordered_list.push(message.id.clone());
        self.total_count += 1;
        self.total_bytes += size_of_val(&message) as u64;
        if self.total_count > self.fetch_cache_limit_count || self.total_bytes > self.fetch_cache_limit_bytes {
            self.cache_full = true;
        }
        if let Some(thid) = message.thid {
            self.thid_lookup.insert(thid, message.id.clone());
        }
        debug!("Message inserted into cache: id({}) cached_count({})", message.id, self.total_count);
    }
    ...
    fn get(&mut self, msg_id: &str) -> Option<(Message, UnpackMetadata)> {
        let r = if let Some((message, meta)) = self.messages.get(msg_id) {
            ...
            if let Some((message, meta)) = self.messages.get(id) {
                ...
                warn!("thid_lookup found message ID ({}) but message id ({}) not found in cache", msg_id, id);
                None
            }
        } else {
            debug!("Message ID ({}) not found in cache, adding to search list", msg_id);
    ...
    fn remove(&mut self, msg_id: &str) -> Option<(Message, UnpackMetadata)> {
        ...
        if self.cache_full && (self.total_count <= self.fetch_cache_limit_count && self.total_bytes <= self.fetch_cache_limit_bytes) {
