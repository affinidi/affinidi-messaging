...
use affinidi_messaging_sdk::{
    ...
    messages::sending::InboundMessageResponse, ATM,
};
use clap::Parser;
use serde_json::json;
use std::error::Error;
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    atm.add_secret(secret_from_str(
        &format!("{}#key-1", sender_did),
        &verification_key1,
    ));
    atm.add_secret(secret_from_str(
        &format!("{}#key-2", sender_did),
        &encryption_key1,
    ));
    ...
    // Pack the message
    let (msg, _) = atm
        .pack_encrypted(&msg, mediator_did, Some(sender_did), Some(sender_did))
        .await
